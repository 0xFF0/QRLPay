...
class Wallet(object):
    ...
    def make_wallet_get_request(self, method):
        ...
        r.raise_for_status()
        current_app.logger.info(f'GET - {self.url} - {method}')
        if r.status_code != 200:
            current_app.logger.error(f'ERROR GET - {self.url} - {method} - {r.status_code}')
            return ""
        else:
            if "error" in r.json():
                current_app.logger.error(f'ERROR GET - {self.url} - {method}')
                current_app.logger.error(r.json()['error'])
                return ""
            else:
                return r.json()
    ...
    def make_wallet_post_request(self, method, params={}, timeout=120):
        url = self.url + method
        current_app.logger.info(f'Request to wallet: {url}')
        r = requests.post(
            url,
        ...
        r.raise_for_status()
        current_app.logger.info(f'POST - {self.url} - {method}')
        if r.status_code != 200:
            current_app.logger.error(f'ERROR POST - {self.url} - {method} - {r.status_code}')
            return ""
        else:
            if "error" in r.json():
                current_app.logger.error(f'ERROR POST - {self.url} - {method}')
                current_app.logger.error(r.json()['error'])
                return ""
            else:
                return r.json()
    ...
    def transfer(self, addr_from, addr_to, amount, fee=10):
    ...
    def get_transfers(self, address):
        data = {'address': address}
        current_app.logger.info(f'get_transfers - {address}')
...
class Coingecko(object):
    ...
    def get_coin_price(self, cur='usd'):
        ...
        current_app.logger.info(f'Coin price - {amt}')
        return amt
...
wallet = Wallet(config.QRL_WALLET_API_HOST, config.QRL_WALLET_API_PORT)
