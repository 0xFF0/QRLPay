...
function payment(payment_data) {
    $('document').ready(function(){
        ...
        $.get("/api/createpayment", invoiceData).then(function(data) {
            ...
            $('#amount').text(invoice.qrl_value).html();
            // $('#amount_sats').text(Math.round(invoice.qrl_value * 10**8)).html();
...
function complete_payment(payment_uuid, payment_data) {
    if (location.search.includes("redirect")) { 
        var order_id = location.search.slice(1).split("&")[2].split("=")[1];
    }
    else {
